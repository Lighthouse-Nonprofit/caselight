class FamilyGrid
  ...
  filter(:family_type, :enum, select: -> { Family::TYPE_LABELS.map { |value, label| [label, value] } }, header: -> { I18n.t('datagrid.columns.families.family_type') }) do |value, scope|
  ...
  column(:family_type, header: -> { I18n.t('datagrid.columns.families.family_type') }) do |object|
    Family.type_label(object.family_type)
  end
