...
| User | google_refresh_token | Credential (Google Calendar offline access, rebuilt push — data-task batch C5) | **Encrypted (Tier 6, non-det; born-encrypted)** | Written only by the OAuth callback; paper_trail-skipped; cleared on disconnect; dormant unless GOOGLE_CLIENT_ID/SECRET are configured |
