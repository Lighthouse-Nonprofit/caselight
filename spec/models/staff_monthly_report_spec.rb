describe 'Staff Monthly Report' do
  # Frozen mid-month: this file's fixtures anchored on bare Date.today (UTC) while the
  # model windows on 1.month.ago in the app zone (America/Los_Angeles) — inside the
  # UTC-next-day window (e.g. UTC Aug 1 00:00–07:00 = LA Jul 31 evening) "last month"
  # DIFFERED between them and all seven examples fell over; month-end runs also flipped
  # duration parity. A fixed mid-month instant kills both classes for good.
  around do |example|
    travel_to(Time.zone.local(2026, 6, 17, 10, 0)) { example.run }
  end
  ...
  let!(:user_1_tasks) { create_list(:task, 2, :incomplete, completion_date: Time.zone.today.last_month.beginning_of_month, users: [user_1]) }
  let!(:user_2_tasks) { create_list(:task, 16, :incomplete, completion_date: Time.zone.today.last_month.beginning_of_month, users: [user_2]) }
  let!(:user_3_tasks) { create(:task, :incomplete, completion_date: Time.zone.today.last_month.end_of_month, users: [user_3]) }
  ...
  let!(:case_note) { create(:case_note, client: client_1, created_at: Time.zone.today.last_month) }
  let!(:case_note_1) { create(:case_note, client: client_1, created_at: Time.zone.today.last_month) }
  let!(:case_notes) { create_list(:case_note, 5, client: client_2, created_at: Time.zone.today.last_month) }
  let!(:case_note_2) { create(:case_note, client: user_4.clients.last, created_at: Time.zone.today.last_month) }
  ...
  let!(:assessment_1) { create(:assessment, client: client_1, created_at: (Time.zone.today.last_month - 6.months)) }
  let!(:assessment_2) { create(:assessment, client: client_1, created_at: Time.zone.today.last_month) }
  let!(:assessment_3) { create(:assessment, client: client_3, created_at: (2.days.ago.last_month - 6.months)) }
  let!(:assessment_4) { create(:assessment, client: client_3, created_at: Time.zone.today.last_month) }

  let!(:assessment_5) { create(:assessment, client: client_5, created_at: Time.zone.today.last_month - 6.months) }
  let!(:assessment_6) { create(:assessment, client: client_5, created_at: Time.zone.today.last_month) }

  let!(:user_1_visit_clients) { create_list(:visit_client, 3, user: user_1, created_at: Time.zone.today.last_month) }
  let!(:user_4_visit_clients) { create_list(:visit_client, 2, user: user_4, created_at: Time.zone.today.last_month) }
  ...
  feature 'report' do
    ...
    scenario 'average number of due today tasks each day' do
      TaskHistory.update_all(created_at: Time.zone.today.last_month, updated_at: Time.zone.today.last_month)
