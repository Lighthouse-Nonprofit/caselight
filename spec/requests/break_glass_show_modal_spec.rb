...
# UX rung 5 — the show-page break-glass affordance: emergency_only forms with data render as
# LOCKED cards in the Overview Forms pane for break-glass-eligible viewers, each opening a
# declarative BS5 modal that embeds the shared break_glass_grants/_form. Contracts:
#   * eligible viewer sees the locked card + a modal carrying the exact grant POST fields
#   * the masked form is NEVER linked to edit from the overview (edit has no field gate)
#   * POSTing the grant unlocks the form (title joins the Forms lists) + the header shows the
#     expiry chip
#   * a strategic_overviewer (never eligible) sees neither locked card nor modal
RSpec.describe 'Break-glass locked cards + modal on clients#show', type: :request do
  ...
  describe 'eligible viewer (case worker)' do
    ...
    it 'renders the locked card and a modal with the exact grant POST fields — and never an edit link' do
      get client_path(client)
      expect(response).to have_http_status(:ok)
      body = response.body

      # locked card affordance
      expect(body).to include('client-overview__locked')
  ...
  describe 'strategic_overviewer (never break-glass eligible)' do
    it 'sees neither locked card nor modal nor sentinel' do
      sign_in_as(overviewer)
      get client_path(client)
      expect(response).to have_http_status(:ok)
      body = response.body

      expect(body).not_to include('client-overview__locked')
